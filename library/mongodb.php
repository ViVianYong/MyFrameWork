 ...
 *
 * 参数：array('id'=>array('$gt' => 5))
 * 等同：where id>5
 *
 * 参数：array('id'=>array('$ne' => 5))
 * 等同：where id!=5
 *
 * 参数：array('id'=>array('$gt' => 5, '$lt' => 10))
 * 等同：where id>5 and id<10
 *
 * 参数：array('id'=>array('$in' => array(2,5,6)))
 * 等同：where id in (2,5,6)
 *
 * 参数：array('$or' => array('$gt' => 5, '$lt' < 10))
...
class MongodbClass {
    ...
    public function insert($colName, $sets, $safe=false, $fsync=false){
        $col = $this->_getCollection($colName);
        try {
    ...
    public function save($colName, $sets, $safe=false, $fsync=false) {
        ...
        $ret = $this->_getCollection($colName)->save($sets,array('w'=>$safe,'fsync'=>$fsync));
        return $ret;
    }
    ...
    public function dropCol($colName){
        return $this->_getCollection($colName)->drop();
    }
     ...
     * 获取索引信息
     */
    public function getIndexs($colName) {
        return $this->_getCollection($colName)->getIndexInfo();
    }
    ...
    public function count($colName,$query=array(),$limit=0,$skip=0){
        return $this->_getCollection($colName)->count($query,$limit,$skip);
    }
    ...
    // 得到集合对象
    private function _getCollection($colName){
